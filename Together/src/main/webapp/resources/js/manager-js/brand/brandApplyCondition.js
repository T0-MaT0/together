...
const filterCustomerStatus = (e)=>{
    ...
    if (customerState == '대기') {
        ...
        location.href = '/manageBrand/apply';
        return;
    }
    
    console.log(customerState)
    fetch("brandApplyCondition?customerState="+ customerState )
    .then(resp=>resp.json())
    .then(customerList =>{
        ...
        if(customerList.pagination.endPage == 0) {
            ...
            notFound.innerText = "제휴가 존재하지 않습니다.";
...
function statePageMove(customerState, cp){
    ...
    fetch("brandApplyCondition?customerState="+customerState+"&cp="+cp )
    .then(resp=>resp.json())
    .then(customerList =>{
        console.log(customerList);
        if(customerList.pagination.endPage == 0) {
